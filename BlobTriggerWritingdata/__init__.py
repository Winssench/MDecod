...
def main(myblob: func.InputStream, outputblob: func.Out[bytes], context: func.Context):
    ...
    with open(filename, 'wb') as f:
        f.write(imgdata)
    '''
    #outputblob.set(imgdata)
    # outputblob.__format__()
    '''
    presentTime = datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%SZ')
    print(presentTime)
    deviceID = 'ISHJCU67'
    path = 'images/'+ deviceID+'/'+presentTime+'.jpg'
    '''
    presentTime = datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%SZ')
    # Use the BlockBlobService

    # Convert the semicolon seperated string to a dict
    ...
    '''
    temp_obj = dict(item.split("=", 1)
                    for item in myStorageBlob_settings.split(";"))
    bbs = BlobServiceClient(
        account_name=temp_obj["storeageaccountteamcbn"],
        account_key=temp_obj["Ehc1pb8txcuLn3pgJ4LuzIqDBEZJyZA7lVsMl8c8BTQ9e1wuXx9HtKrqHR8Mf63hduufeYCWJA9199Ebo7/vFg=="]
    )
    '''
